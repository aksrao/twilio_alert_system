...
__TableName__ = "twilio_db" # __TableName__ = abc
...
@application.route('/insert', methods=['post']) # new
def insert():
    userinfo=session[constants.PROFILE_KEY]
...
# @application.route('/update_item/<id>/<name>', methods=['post'])
...
@application.route('/update_item/<id>', methods=['post'])
def update_item(id):
    if request.method == 'POST':
        table = DB.Table('twilio_db')
        table.update_item(
...
if __name__ == "__main__":
    application.run(host='0.0.0.0', port=env.get('PORT', 80))